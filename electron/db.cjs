const path = require('path')
const fs = require('fs')

// データベースのデフォルト構造
const defaultData = {
  categories: [],
  settings: {
    maxQueueSize: 100,
    autoCleanup: true
  }
}

// データベースファイルのパス（Electronの場合はapp.getPath('userData')を使用）
let dbPath = null
let appInstance = null

// Electronアプリインスタンスを設定
function setApp(app) {
  appInstance = app
  // Electron環境では userData ディレクトリを使用
  dbPath = path.join(app.getPath('userData'), 'data', 'db.json')
}

// データベースの初期化（シングルトン）
let dbPromise = null

async function getDB() {
  if (!dbPromise) {
    // データベースパスが設定されていない場合はデフォルトを使用
    if (!dbPath) {
      dbPath = path.join(process.cwd(), 'data', 'db.json')
    }
    
    // データディレクトリが存在しない場合は作成
    const dataDir = path.dirname(dbPath)
    if (!fs.existsSync(dataDir)) {
      fs.mkdirSync(dataDir, { recursive: true })
    }
    
    // lowdbをダイナミックインポート
    const { JSONFilePreset } = await import('lowdb/node')
    dbPromise = JSONFilePreset(dbPath, defaultData)
  }
  const db = await dbPromise
  // 毎回ファイルから最新データを読み込む
  await db.read()
  return db
}

// カテゴリ関連の操作
async function getCategories() {
  try {
    const database = await getDB()
    return { success: true, data: database.data.categories.sort((a, b) => a.index - b.index) }
  } catch (error) {
    return { success: false, error: error.message }
  }
}

async function getCategoryById(categoryId) {
  try {
    const database = await getDB()
    const category = database.data.categories.find(cat => cat.id === categoryId)
    return { success: true, data: category }
  } catch (error) {
    return { success: false, error: error.message }
  }
}

async function addCategory(categoryData) {
  try {
    const database = await getDB()
    await database.update(({ categories }) => {
      const maxIndex = categories.length > 0 
        ? Math.max(...categories.map(c => c.index)) 
        : -1
      const newCategory = {
        id: Date.now().toString(),
        createdAt: new Date().toISOString(),
        index: maxIndex + 1,
        queues: [],
        ...categoryData
      }
      categories.push(newCategory)
    })
    return { success: true, data: database.data.categories[database.data.categories.length - 1] }
  } catch (error) {
    return { success: false, error: error.message }
  }
}

async function updateCategory(categoryId, updates) {
  try {
    const database = await getDB()
    await database.update(({ categories }) => {
      const index = categories.findIndex(cat => cat.id === categoryId)
      if (index !== -1) {
        categories[index] = { 
          ...categories[index], 
          ...updates, 
          updatedAt: new Date().toISOString() 
        }
      }
    })
    const category = database.data.categories.find(cat => cat.id === categoryId)
    return { success: true, data: category }
  } catch (error) {
    return { success: false, error: error.message }
  }
}

async function deleteCategory(categoryId) {
  try {
    const database = await getDB()
    await database.update(({ categories }) => {
      const index = categories.findIndex(cat => cat.id === categoryId)
      if (index !== -1) {
        categories.splice(index, 1)
      }
    })
    return { success: true }
  } catch (error) {
    return { success: false, error: error.message }
  }
}

async function reorderCategories(categoryIds) {
  try {
    const database = await getDB()
    await database.update(({ categories }) => {
      categoryIds.forEach((id, newIndex) => {
        const category = categories.find(cat => cat.id === id)
        if (category) {
          category.index = newIndex
        }
      })
    })
    return { success: true }
  } catch (error) {
    return { success: false, error: error.message }
  }
}

// キュー関連の操作
async function addQueue(categoryId, queueData) {
  try {
    const database = await getDB()
    await database.update(({ categories }) => {
      const category = categories.find(cat => cat.id === categoryId)
      if (category) {
        const maxIndex = category.queues.length > 0 
          ? Math.max(...category.queues.map(q => q.index)) 
          : -1
        const newQueue = {
          id: Date.now().toString(),
          createdAt: new Date().toISOString(),
          index: maxIndex + 1,
          ...queueData
        }
        category.queues.push(newQueue)
      }
    })
    const category = database.data.categories.find(cat => cat.id === categoryId)
    return { success: true, data: category ? category.queues[category.queues.length - 1] : null }
  } catch (error) {
    return { success: false, error: error.message }
  }
}

async function getQueuesByCategory(categoryId) {
  try {
    const database = await getDB()
    const category = database.data.categories.find(cat => cat.id === categoryId)
    return { success: true, data: category ? category.queues.sort((a, b) => a.index - b.index) : [] }
  } catch (error) {
    return { success: false, error: error.message }
  }
}

async function getAllQueues() {
  try {
    const database = await getDB()
    const allQueues = []
    database.data.categories.forEach(category => {
      category.queues.forEach(queue => {
        allQueues.push({
          ...queue,
          categoryId: category.id,
          categoryName: category.name
        })
      })
    })
    return { success: true, data: allQueues }
  } catch (error) {
    return { success: false, error: error.message }
  }
}

async function getQueueById(categoryId, queueId) {
  try {
    const database = await getDB()
    const category = database.data.categories.find(cat => cat.id === categoryId)
    const queue = category ? category.queues.find(queue => queue.id === queueId) : null
    return { success: true, data: queue }
  } catch (error) {
    return { success: false, error: error.message }
  }
}

async function updateQueue(categoryId, queueId, updates) {
  try {
    const database = await getDB()
    await database.update(({ categories }) => {
      const category = categories.find(cat => cat.id === categoryId)
      if (category) {
        const index = category.queues.findIndex(queue => queue.id === queueId)
        if (index !== -1) {
          category.queues[index] = { 
            ...category.queues[index], 
            ...updates, 
            updatedAt: new Date().toISOString() 
          }
        }
      }
    })
    const category = database.data.categories.find(cat => cat.id === categoryId)
    const queue = category ? category.queues.find(q => q.id === queueId) : null
    return { success: true, data: queue }
  } catch (error) {
    return { success: false, error: error.message }
  }
}

async function deleteQueue(categoryId, queueId) {
  try {
    const database = await getDB()
    await database.update(({ categories }) => {
      const category = categories.find(cat => cat.id === categoryId)
      if (category) {
        const index = category.queues.findIndex(queue => queue.id === queueId)
        if (index !== -1) {
          category.queues.splice(index, 1)
        }
      }
    })
    return { success: true }
  } catch (error) {
    return { success: false, error: error.message }
  }
}

async function reorderQueues(categoryId, queueIds) {
  try {
    const database = await getDB()
    await database.update(({ categories }) => {
      const category = categories.find(cat => cat.id === categoryId)
      if (category) {
        queueIds.forEach((id, newIndex) => {
          const queue = category.queues.find(q => q.id === id)
          if (queue) {
            queue.index = newIndex
          }
        })
      }
    })
    return { success: true }
  } catch (error) {
    return { success: false, error: error.message }
  }
}

async function moveQueue(fromCategoryId, toCategoryId, queueId, newIndex) {
  try {
    const database = await getDB()
    await database.update(({ categories }) => {
      const fromCategory = categories.find(cat => cat.id === fromCategoryId)
      const toCategory = categories.find(cat => cat.id === toCategoryId)
      
      if (fromCategory && toCategory) {
        const queueIndex = fromCategory.queues.findIndex(q => q.id === queueId)
        if (queueIndex !== -1) {
          // 移動元カテゴリからキューを削除
          const [queue] = fromCategory.queues.splice(queueIndex, 1)
          
          // 移動先カテゴリに追加し、インデックスを再計算
          toCategory.queues.push(queue)
          const maxIndex = toCategory.queues.length > 0 
            ? Math.max(...toCategory.queues.map(q => q.index || 0)) 
            : -1
          queue.index = maxIndex + 1
          
          // 移動元カテゴリのインデックスを再計算
          fromCategory.queues.forEach((q, idx) => {
            q.index = idx
          })
          
          // 移動先カテゴリのインデックスを再計算（必要な場合）
          if (typeof newIndex === 'number' && newIndex >= 0) {
            // 指定されたインデックスに移動
            toCategory.queues.sort((a, b) => a.index - b.index)
            const movedQueue = toCategory.queues.pop()
            toCategory.queues.splice(newIndex, 0, movedQueue)
            toCategory.queues.forEach((q, idx) => {
              q.index = idx
            })
          }
        }
      }
    })
    return { success: true }
  } catch (error) {
    return { success: false, error: error.message }
  }
}

// 設定関連の操作
async function updateSettings(newSettings) {
  try {
    const database = await getDB()
    await database.update(({ settings }) => {
      Object.assign(settings, newSettings)
    })
    return { success: true, data: database.data.settings }
  } catch (error) {
    return { success: false, error: error.message }
  }
}

async function getSettings() {
  try {
    const database = await getDB()
    return { success: true, data: database.data.settings }
  } catch (error) {
    return { success: false, error: error.message }
  }
}

module.exports = {
  setApp,
  getCategories,
  getCategoryById,
  addCategory,
  updateCategory,
  deleteCategory,
  reorderCategories,
  addQueue,
  getQueuesByCategory,
  getAllQueues,
  getQueueById,
  updateQueue,
  deleteQueue,
  reorderQueues,
  moveQueue,
  updateSettings,
  getSettings
}

